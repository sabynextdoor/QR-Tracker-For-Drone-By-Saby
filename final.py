...
class SeedMatcher:
    ...
    def analyze_orientation(self, qr_corners):
        """Analyze QR orientation and provide feedback with direction"""
        ...
        try:
            ...
            # Generate detailed feedback with direction
            feedback = []
            ...
            if not is_angle_ok:
                if angle_diff > 0:
                    ...
                    feedback.append(f"Rotate QR {rotation_amount:.0f}° CLOCKWISE (→)")
                    feedback.append(f"Turn the QR code to your RIGHT")
                else:
                    rotation_direction = "counter-clockwise"
                    feedback.append(f"Rotate QR {rotation_amount:.0f}° COUNTER-CLOCKWISE (←)")
                    feedback.append(f"Turn the QR code to your LEFT")
            
            if not is_perspective_ok:
                if aspect_diff > 0:
                    feedback.append("Tilt camera UPWARD (↑)")
                    feedback.append("Move camera higher or tilt QR down")
                else:
                    feedback.append("Tilt camera DOWNWARD (↓)")
                    feedback.append("Move camera lower or tilt QR up")
            
            # Determine if QR is skewed
            skew = self._calculate_skew(corners)
            if skew > 0.15:  # 15% skew is too much
                feedback.append("Hold QR FLAT (reduce tilt)")
                feedback.append("Bring QR parallel to camera")
            ...
            return {
                ...
                'feedback': feedback,
                'status': 'ADJUST ROTATION' if not is_angle_ok else ('ADJUST ANGLE' if not is_perspective_ok else 'OK'),
                'skew': skew
            }
...
def draw_rotation_arrow(frame, center, direction, angle_diff):
    """Draw a rotation arrow showing which way to turn"""
    cx, cy = center
    radius = 80
    
    if direction == "clockwise":
        # Draw clockwise arrow (right turn)
        cv2.ellipse(frame, (cx, cy), (radius, radius), 0, 0, 270, (0, 165, 255), 4)
        # Arrow head
        end_angle = math.radians(270)
        tip_x = int(cx + radius * math.cos(end_angle))
        tip_y = int(cy + radius * math.sin(end_angle))
        cv2.arrowedLine(frame, (tip_x, tip_y), (tip_x - 20, tip_y - 20), (0, 165, 255), 4, cv2.LINE_AA)
        
        # Text
        cv2.putText(frame, f"ROTATE → {abs(angle_diff):.0f}°", (cx - 60, cy - 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, "CLOCKWISE", (cx - 50, cy - 65),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2, cv2.LINE_AA)
    else:
        # Draw counter-clockwise arrow (left turn)
        cv2.ellipse(frame, (cx, cy), (radius, radius), 0, 270, 360, (0, 165, 255), 4)
        # Arrow head
        end_angle = math.radians(270)
        tip_x = int(cx + radius * math.cos(end_angle))
        tip_y = int(cy + radius * math.sin(end_angle))
        ...
        # Text
        cv2.putText(frame, f"ROTATE ← {abs(angle_diff):.0f}°", (cx - 80, cy - 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, "COUNTER-CLOCKWISE", (cx - 70, cy - 65),
...
def draw_overlay(frame, pts, last_data, match_info=None, orientation_info=None):
    ...
    # Draw rotation arrow if orientation is wrong
    if orientation_info and not orientation_info.get('is_angle_ok', True):
    ...
    if last_data:
        if match_info and match_info.get('is_match', False):
            if orientation_info and not orientation_info.get('is_angle_ok', True):
                # Show specific rotation instruction
                if orientation_info.get('rotation_direction') == "clockwise":
                    label = f"⚠ ROTATE RIGHT (CLOCKWISE) {orientation_info.get('rotation_amount', 0):.0f}° ⚠"
                else:
                    label = f"⚠ ROTATE LEFT (COUNTER-CLOCKWISE) {orientation_info.get('rotation_amount', 0):.0f}° ⚠"
...
def main():
    ...
    while True:
        ...
        if match_info and match_info.get('is_match', False):
            ...
            if orientation_info and (current_time - last_feedback_time) > feedback_cooldown:
                if not orientation_info.get('is_angle_ok', True):
                    print("\n" + "="*55)
                    print("⚠ ROTATION INSTRUCTION ⚠")
                    print("="*55)
                    for feedback in orientation_info.get('feedback', []):
                        print(f"  {feedback}")
                    print(f"\n  Current angle: {orientation_info['current_angle']:.1f}°")
                    print(f"  Target angle: {orientation_info['target_angle']:.1f}°")
                    if orientation_info['angle_diff'] > 0:
                        print(f"  → Turn QR {abs(orientation_info['angle_diff']):.1f}° to the RIGHT")
                    else:
                        print(f"  ← Turn QR {abs(orientation_info['angle_diff']):.1f}° to the LEFT")
                    print("="*55 + "\n")
        ...
        if raw_pts is not None:
            ...
            if is_valid_qr(ordered, fw, fh):
                ...
                if data and data != last_data:
                    ...
                    if match_info and match_info.get('is_match', False):
                        if orientation_info and not orientation_info.get('is_angle_ok', True):
                            direction = "RIGHT" if orientation_info.get('rotation_direction') == "clockwise" else "LEFT"
                            print(f"  [MATCHED - WRONG ROTATION]  {data}")
                            print(f"  → Turn {direction} {orientation_info.get('rotation_amount', 0):.0f}°")
                        else:
        ...
        if display_pts is not None:
            ...
            if last_match_info and last_match_info.get('is_match', False):
                if last_orientation and not last_orientation.get('is_angle_ok', True):
                    s_txt, s_col = "⚠ ROTATE QR TO MATCH ⚠", (0, 165, 255)
                else:
        ...
        cv2.putText(frame, s_txt,(12,42),
                    cv2.FONT_HERSHEY_SIMPLEX,1.0,s_col,2,cv2.LINE_AA)
        ...
        if last_orientation and last_match_info and last_match_info.get('is_match', False):
            if not last_orientation.get('is_angle_ok', True):
                # Show clear rotation instruction in top bar
                if last_orientation.get('rotation_direction') == "clockwise":
                    rot_text = f"→ ROTATE RIGHT {last_orientation.get('rotation_amount', 0):.0f}° →"
                else:
                    rot_text = f"← ROTATE LEFT {last_orientation.get('rotation_amount', 0):.0f}° ←"
                cv2.putText(frame, rot_text,(12,75),
                            cv2.FONT_HERSHEY_SIMPLEX,0.65,(0,165,255),2,cv2.LINE_AA)
        
        # Seed info
        if seed_matcher.seed_loaded:
            seed_short = seed_matcher.seed_data[:25] + "..."
            cv2.putText(frame, f"SEED: {seed_short}",(fw-400,28),
                        cv2.FONT_HERSHEY_SIMPLEX,0.45,(0,200,100),1,cv2.LINE_AA)
        
        # Camera FPS
        cv2.putText(frame, f"CAM: {cam_fps:.0f} fps",(fw-150,42),
